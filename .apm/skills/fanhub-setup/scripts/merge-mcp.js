...
try {
  ...
  const fanhubServers = JSON.parse(
    fs.readFileSync(mcpServersFile, "utf8"),
  ).mcpServers;
  ...
  if (fs.existsSync(mcpJsonFile)) {
    try {
      ...
      console.warn(
        `Warning: Could not parse existing ${mcpJsonFile}, will create new one`,
      );
    }
  }
  ...
  console.log(
    `  - Added/Updated ${Object.keys(fanhubServers).length} server(s)`,
  );
  console.log(
    `  - Total servers now: ${Object.keys(mcpConfig.mcpServers).length}`,
  );
  console.log("");
  console.log("✓ Setup complete! Your MCP configuration is ready.");
  console.log(
    "  Next: Start your FanHub backend (e.g., cd dotnet/Backend && dotnet run)",
  );
